...
if __name__ == "__main__":
    ...
    각 특징 맵이 잠쟂거으로 다른 사이즈와 측면 비율을 가질 수 있기 때문에 Tuple[Tuple[int]] 타입을 가지도록 합니다.
    """
    anchor_generator = AnchorGenerator(sizes=((32, 64, 128, 256, 512), ),
                                       aspect_ratios=((0.5, 1.0, 2.0), ))
    """
    관심 영역의 자르기 및 재할당 후 자르기 크기를 수행하는 데 사용할 피쳐 맵을 정의합니다.
    만약 백본이 텐서를 리턴할 때, featmap_names는 [0]이 될 것이라고 예상합니다.
    ...
    def main():
        ...
        # 데이터셋을 학습용과 테스트용으로 나눕니다. (역자주: 여기서는 전체의 50개를 테스트에, 나머지를 학습에 사용)
        indices = torch.randperm(len(dataset)).tolist()
        dataset = torch.utils.data.Subset(dataset, indices[:-50])
        dataset_test = torch.utils.data.Subset(dataset_test, indices[-50:])

        # 데이터 로더를 학습용과 검증용으로 정의합니다.
        data_loader = torch.tuils.data.DataLoader(dataset,
                                                  batch_size=2,
                                                  shuffle=True,
                                                  num_workers=4,
